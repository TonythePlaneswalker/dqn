...
def get_model(state, num_actions, scope):
    with tf.variable_scope(scope):
        ...
        v_net = tf.contrib.layers.fully_connected(v_net, 1, activation_fn=None, scope='v_net/fc1')
        a_net = tf.contrib.layers.fully_connected(feature, 512, scope='a_net/fc_0')
        a_net = tf.contrib.layers.fully_connected(a_net, num_actions, activation_fn=None, scope='a_net/fc_1')
